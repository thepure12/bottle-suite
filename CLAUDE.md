...
- `get` supports filtering by any query param (`LIKE`), a single-row lookup by primary key, a `levels` param for following foreign keys N levels deep via `getRefs` (which queries `information_schema` for FK relationships and recursively inlines referenced rows), and nested-resource routes like `/<ref_table>/<ref_id>/<table>`. **`getRefs`'s FK-discovery query only understands MySQL's `information_schema` syntax — there is no sqlite equivalent anywhere in the codebase.** Any call that needs to run that discovery query (default `levels=1` skips it for a single row lookup, but the `/<ref_table>/<ref_id>/<table>` nested-route path always runs it, and `levels>=2` triggers it whenever the per-class `refs` cache is empty) raises `sqlite3.OperationalError: no such table: information_schema.table_constraints` on a sqlite-backed app. Nested-ref routes only actually work against MySQL.
...
- `app.py` — the `bottle-suite` entry point; parses CLI flags (`--jwt`, `--sqlite`, `--dbhost/--dbname/--dbuser/--dbpass`, `--cors`, `-r`, `-d`), builds `BottleSuite(**kwargs)`, retries on `Exception` with a 5s backoff (but exits on `sqlite3.OperationalError`/`pymysql.err.OperationalError`), and cleans up the temp SQLite file on exit if `--sqlite` was used with no path.
